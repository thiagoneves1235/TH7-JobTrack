const JobTrackAPI = (() => {
  const storagePrefix = "th7-jobtrack-applications";
  const seed = [
  ...
  const workspaceKey = () => {
    ...
    const identity = session?.email || "guest";
    return `${storagePrefix}:${identity.toLowerCase().replace(/[^a-z0-9-]/g, "-")}`;
  };
  const read = () => JSON.parse(localStorage.getItem(workspaceKey())) || seed;
  const write = (items) => localStorage.setItem(workspaceKey(), JSON.stringify(items));
  return {
    list: () => read(),
